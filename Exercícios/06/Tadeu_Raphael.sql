...
Comentários: Não consegui terminar a lista, muitos exercícios eu não soube fazer

*/

/* Parte 01 - Não soube fazer */
...
SELECT *
FROM Imovel
WHERE ds_Imovel > ALL(SELECT AVG(ds_Imovel) FROM Imovel);

/* Parte 03 */

SELECT *
FROM Comprador
WHERE cd_Comprador = ANY(SELECT cd_Comprador FROM Oferta WHERE vl_Oferta > 70000);

/* Parte 04 */

SELECT *
FROM Imovel
WHERE ds_Imovel > ALL(SELECT AVG(vl_Oferta) FROM Oferta);

/* Parte 05 - Não soube fazer novamente */
...
Comentários: Mais uma lista que não consegui fazer pois estou  com dificuldade na matéria. Estarei enviando ela completa antes de finalizar a semana.

*/

/* Parte 01 */

CREATE PROCEDURE Acrescimo (Bairro VARCHAR(20), Valor INT)
BEGIN
UPDATE ds_Imovel 
FROM Imovel
WHERE ds_Imovel = (ds_Imovel * (@Valor))
END
...
Comentários: Como no primeiro exercício não existe uma categorização específica para que coluna ou linha era para modificar o valor,
eu fiz um sistema só de teste, onde cada valor inserido, excluido ou modificado vai tirar 1 do mesmo valor inserido. 

Nesse exercício tive problemas em especial nos comandos DELETE. 
...
CREATE TRIGGER tr_insert_atualizacao BEFORE INSERT
ON Imovel
FOR EACH ROW
SET NEW.ds_Imovel = (NEW.ds_Imovel - 1);

/* Modificado */

CREATE TRIGGER tr_update_atualizacao BEFORE UPDATE
ON Imovel
FOR EACH ROW
SET NEW.ds_Imovel = (NEW.ds_Imovel - 1);

/* Excluido - Não está funcionando */

CREATE TRIGGER tr_delete_atualizacao AFTER DELETE
ON Imovel
FOR EACH ROW
SET OLD.ds_Imovel = OLD.ds_Imovel;

/* Parte 02 */

CREATE TRIGGER tr_update_impedir BEFORE UPDATE
ON Estado
FOR EACH ROW
SET NEW.sg_Estado = OLD.sg_Estado AND NEW.nm_Estado = OLD.nm_Estado;

CREATE TRIGGER tr_delete_impedir BEFORE DELETE
ON Estado
FOR EACH ROW
SET NEW.sg_Estado = OLD.sg_Estado AND NEW.nm_Estado = OLD.sg_Estado;

/* Parte 03 */

CREATE TRIGGER tr_update_impedir2 BEFORE UPDATE
ON Faixa_Imovel
FOR EACH ROW
SET NEW.ROW = OLD.ROW;

CREATE TRIGGER tr_delete_impedir2 BEFORE DELETE
ON Faixa_Imovel
FOR EACH ROW
SET NEW.sg_Estado = OLD.sg_Estado AND NEW.nm_Estado = OLD.sg_Estado;